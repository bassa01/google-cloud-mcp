...
describe("output utilities", () => {
  ...
  it("builds preview notes depending on totals, omissions, and limits", () => {
    ...
    expect(buildPreviewNote({ total: 5, displayed: 5, label: "rows" })).toBeUndefined();
  ...
  it("trims text previews and annotates truncation", () => {
    const short = createTextPreview("abc", 10);
    expect(short).toEqual({ text: "abc", truncated: false });

    const long = createTextPreview("abcdefghijklmnopqrstuvwxyz", 5);
    expect(long.text).toBe("abcde…");
    expect(long.truncated).toBe(true);
  });

  it("resolves numeric environment inputs with fallbacks and bounds", () => {
    expect(resolveBoundedNumber(undefined, 10, { min: 1, max: 20 })).toBe(10);
    expect(resolveBoundedNumber("not-a-number", 8, { min: 1, max: 20 })).toBe(8);
    expect(resolveBoundedNumber("100", 8, { min: 1, max: 50 })).toBe(50);
    expect(resolveBoundedNumber("4.9", 8, { min: 5, max: 10 })).toBe(5);
  });
});
