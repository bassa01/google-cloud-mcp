...
- `gcp-spanner-query-stats` (リソース) – `SPANNER_SYS.QUERY_STATS_TOP_MINUTE/10MINUTE/HOUR` を読み、1m/10m/1h のレイテンシー/CPU トップクエリを Markdown ヒートマップで提示。
...
| Spanner | `gcp-spanner-query-stats` (resource) | Query Insights を 1m/10m/1h ヒートマップで表示。 |
