...
### gcp-projects-set-project-id — 既定プロジェクト設定
...
### gcp-projects-get-project-id — 既定プロジェクト確認
...
MCP リソースは `read_resource` / `get_resource` で取得します。特に記載がない限り、`{projectId}` や `{instanceId}`、`{databaseId}` などのプレースホルダは `gcp-projects-set-project-id` や Google Cloud 認証から決定された既定値にフォールバックできます。
